...
module.exports = {
    ...
    allmessages: async function (req, res) {
        ...
        let allrooms = await Room.find(roomcondition).populate("user company job").sort({ "$natural": -1 }).lean({ virtuals: true }).exec();

        for (let i = 0; i < allrooms.length; i++) {
            allrooms[i].message = [];
            let message = await Message.find({ room: allrooms[i]._id }).populate("user company").exec();
    ...
    saveapplicantjob: async function (req, res) {
        ...
        const userinfo = await User.findById(userid).sort({ "$natural": -1 }).lean().exec();
        ...
        //        }
        const companyinfo = await Job.findById(jobid).sort({ "$natural": -1 }).lean().exec();
    ...
    companyinfo: async function (req, res) {
        ...
        let userid = req.userid;
        const companyinfo = await Company.findById(companyid).sort({ "$natural": -1 }).exec();
        let today = new Date(new Date().setDate(new Date().getDate() - 1))


        const activejobs = await Job.find({ "company": companyid, 'toduration': { $gt: today } }).populate("user company").sort({ "$natural": -1 }).limit(4).lean().exec();
        let previousjob = await Job.find({ "company": companyid, 'toduration': { $lt: today } }).populate("user company").sort({ "$natural": -1 }).limit(4).lean().exec();

        const msgcount = 1;

        let profileprogress = await UserHelper.calculateProfileProgress(companyinfo, false)
        if (!userid) {
            profileprogress = {}
            previousjob = []
        }
    ...
    companydashboard: async function (req, res) {
        ...
        const companyinfo = await Company.findById(companyid).sort({ "$natural": -1 }).lean({ virtuals: true }).exec();

        let today = new Date(new Date().setDate(new Date().getDate() - 1))
        const countactivejobs = await Job.countDocuments({ company: companyid, 'toduration': { $gt: today } }).sort({ "$natural": -1 }).lean().exec();
        const countapplicant = await JobApplicant.countDocuments({ company: companyid }).sort({ "$natural": -1 }).lean({ virtuals: true }).exec();
        const lastestapplicant = await JobApplicant.find({ company: companyid }).populate("user job").sort({ "$natural": -1 }).limit(4).lean({ virtuals: true }).exec();
        const mostactivecompanies = await UserHelper.mostactivecompanies()
        const mostsearchedskills = await Skill.find().sort({ "$natural": -1 }).limit(4).lean().exec();
    ...
    companyupdate: async function (req, res) {
        ...
        let companyinfo = await Company.findById(companyid).sort({ "$natural": -1 }).exec();
    ...
    companyapplicant: async function (req, res) {
        ...
        const data = await JobApplicant.find({ company: companyid }).populate({
        ...
        }).populate("job").sort({ "$natural": -1 }).lean({ virtuals: true }).exec();
        for (let i = 0; i < data.length; i++) {
            let score = 0;
            if (data[i].user && data[i].user.skills && data[i].user.skills.length > 0) {
                data[i].user.skills = await Skill.find({ _id: data[i].user.skills }).sort({ "$natural": -1 }).lean().exec();
            }
            data[i].user && data[i].user.skills && data[i].user.skills.map(item => {
    ...
    jobsearch: async function (req, res) {
        ...
        if (isjob && excludeNoSalary) {
            ...
            salarycondition.push({ $and: [{ maxsalary: { $lte: 0 } }, { minsalary: { $lte: 0 } }] })
        }
        ...
        if (isjob && maxsalary > 0) { //
            salarycondition.push({ $and: [{ maxsalary: { $gte: maxsalary } }, { minsalary: { $lte: minsalary } }] })
            salarycondition.push({ $and: [{ maxsalary: { $lte: maxsalary } }, { maxsalary: { $gte: minsalary } }] });
                    ...
                    $and: [
                        {
                            fromduration:
                            {
                                $lte: new Date(new Date().setDate(new Date(enddate).getDate() + 1))
        ...
        for (let i = 0; i < skills.length; i++) {
            ...
            if (skills[i].value === skills[i].label) {
                ...
                const data = await Skill.find({ name: { "$regex": key, "$options": "i" } }).sort({ "$natural": -1 }).lean().exec();
                if (data && data.length) {
        ...
        if (containskills) {
          ...
          if (isjob) {
                data = await Job.find({
                    skills: skillcondition, $or: orcondition,
                    $and: andcondition
                }).populate("user company").sort({ "$natural": -1 }).lean().exec();
            } else { // talent
                data = await User.find({ skills: skillcondition, $or: orcondition, $and: andcondition }).populate("type").sort({ "$natural": -1 }).lean({ virtuals: true }).exec();
            }
        } else {
           ...
           // orcondition = [ { jobtype: { '$regex': "Full time", '$options': 'i' } } ];
            ...
            if (isjob) {
                ...
                data = await User.find({
                    $or: orcondition,
                    $and: andcondition
                }).populate("type").sort({ "$natural": -1 }).lean({ virtuals: true }).exec();
            }
        }
        ...
        for (let i = 0; i < data.length; i++) {
            if (data[i].skills.length > 0) {
                data[i].skills = await Skill.find({ _id: data[i].skills }).sort({ "$natural": -1 }).lean().exec();
            }
        }
        return Response.ok(res, data);
    },
